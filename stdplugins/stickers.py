...
@borg.on(admin_cmd(pattern="^.dızla$"))
async def _(event):
    if event.fwd_from:
        return
    if not event.is_reply:
        await event.edit("Sticker paketine ekleme yapmak için \n bir fotoğraf veya çıkartma seç.")
        return  
    reply_message = await event.get_reply_message()
    sticker_emoji = "🔥"
    input_str = event.pattern_match.group(1)
    ...
    me = borg.me
    userid = borg.uid
    user_name = await event.client.get_me()
    packname = f"@{user_name.username}'nın pack v2"
    packshortname = f"a{userid}_by_{user_name.username}_2"  # format: Uni_Borg_userid
    ...
    await event.edit("Çıkartma dızlanıyor..")
    ...
    async with borg.conversation("@Stickers") as bot_conv:
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if is_a_s:
                ...
                response = await silently_send_message(bot_conv, "/newpack")
            if "Yay!" not in response.text:
                await event.edit(f"**HATA**! @Stickers söyledi: {response.text}")
                return
            response = await silently_send_message(bot_conv, packname)
            if not response.text.startswith("Alright!"):
                await event.edit(f"**HATA**! @Stickers söyledi: {response.text}")
    ...
    await event.edit(f"Çıkartma eklendi! Paket [şurada](t.me/addstickers/{packshortname}) bulunabilir.")
